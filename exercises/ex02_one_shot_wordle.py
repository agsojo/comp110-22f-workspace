...
while i < len(secret_word): # loop that compares the indices of the guessed word to the secret
    if user_guess[i] == secret_word[i]: # checks to see if the guess and secret have a matching index
        ...
        while guessed_char == False and j < len(secret_word): # loop to check if the character appears anywhere else in the secret
        ...
        if guessed_char == True: # checks if the previous loop ended because guessed_char became True or if j became too large
